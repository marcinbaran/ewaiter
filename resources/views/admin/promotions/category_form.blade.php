<x-admin.layout.admin-layout>
    ...
    <x-admin.form.form id="tags" formWide="w-1/2" role="form" method="POST" :redirectUrl="$redirectUrl"
        ...
        <div>
            <input type="hidden" name="type" value="{{ \App\Models\Promotion::TYPE_ON_CATEGORY }}">
            ...
            <div class="flex">
                <div class="flex-1">
                    <x-admin.form.new-input containerClass="rounded-r-none" type="percent" id="promotion_percent_value"
                                            name="value" max="99" value="{{ $oldValues['value'] }}" />
                </div>
                <div class="flex-1">
                    <x-admin.form.new-input type="money" containerClass="rounded-r-none" id="promotion_money_value"
                                            name="value" value="{{ $oldValues['value'] }}" />
                </div>
                <x-admin.form.select class="!w-auto rounded-l-none" name="typeValue" :required="true">
                    <option
                        value="0" {{ $oldValues['typeValue'] == \App\Models\Promotion::TYPE_VALUE_PERCEENT ? 'selected' : '' }}>
                        %
                    </option>
                    <option
                        value="1" {{ $oldValues['typeValue'] == \App\Models\Promotion::TYPE_VALUE_PRICE ? 'selected' : '' }}>
                        PLN
                    </option>
                </x-admin.form.select>
            </div>
        ...
        <div>
            <x-admin.form.label for="promotion_orderCategory"
                                :required="true">{{ __('admin.The food category on which we are doing the promotion') }}</x-admin.form.label>
            <x-admin.form.new-input type="select" id="promotion_orderCategory" name="orderCategory[id]"
                                    :value="route('admin.promotions.categories', ['id' => $data->id])"
                                    :oldValue="$oldValues['orderCategory']" :required="true" />
        </div>
        <div class="promotion-dates-container justify-space-between flex flex-col 2xl:flex-row gap-6">
            <div class="flex-1">
                ...
                <x-admin.form.new-input type="date" id="promotion_startAt" name="startAt"
                                        value="{{ $oldValues['startAt'] }}" placeholder="Od dnia"
                                        min="{{ Carbon\Carbon::now()->format('Y-m-d') }}" />
            </div>
            <div class="flex-1">
                <x-admin.form.label for="promotion_endAt">{{ __('admin.End date') }}</x-admin.form.label>
                <x-admin.form.new-input type="date" id="promotion_endAt" name="endAt" value="{{ $oldValues['endAt'] }}"
